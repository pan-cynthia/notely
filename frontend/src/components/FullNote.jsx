...
const FullNote = ({ note, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div
        className="absolute top-0 right-0 w-[500px] h-full bg-white rounded shadow-lg z-50"
        ref={drawerRef}
      >
        <div className="flex flex-col h-full p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-slate-600 text-xs">
            {note.tags.map((tag, index) => (
              <span className="px-2 py-1 bg-slate-100 rounded" key={index}>
                #{tag}
              </span>
            ))}
          </div>
          <button
            className="mt-6 py-2 text-sm text-white bg-blue-500 hover:bg-blue-600 rounded cursor-pointer"
            onClick={onClose}
          >
            Close
          </button>
