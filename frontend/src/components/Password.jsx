...
const Password = ({value, onChange, placeholder}) => {
  ...
  return (
     <div className="flex items-center justify-between w-full px-5 py-3 mb-4 text-sm rounded shadow outline-none">
        ...
        {isShowPassword ? 
        <button className="cursor-pointer" onClick={toggleShowPassword} type="button"><FaRegEye className="text-blue-500" size={20}/></button> :
        <button className="cursor-pointer" onClick={toggleShowPassword} type="button"><FaRegEyeSlash className="text-blue-500" size={20}/></button>}
      </div>
  )
}
