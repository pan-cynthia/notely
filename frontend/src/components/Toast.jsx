...
const Toast = ({ isShown, type, message, onClose }) => {
  ...
  useEffect(() => {
    if (fadeOut) { // wait for animation to complete before calling onClose
      const timer = setTimeout(() => {
        onClose();
      }, 500);
      return () => {
  ...
  return (
    <div className={`fixed top-20 right-6 transition-all duration-500 ease-in-out ${(isShown && !fadeOut) ? "opacity-100 z-50 translate-y-0" : "opacity-0 -z-5 translate-y-4"}`}> {/* set z-index to neg value, otherwise notecard in same spot cannot be pinned */}
