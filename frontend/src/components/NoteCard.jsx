...
const NoteCard = ({
  ...
  return (
    <div className="relative w-full h-35 px-5 py-3 mb-4 text-sm rounded shadow hover:shadow-xl hover:shadow-slate-300 outline-none">
      ...
      >
        <MdOutlinePushPin
          className={`${isPinned ? "text-blue-500" : "text-slate-300"}`}
          size={15}
        />
      </button>
      <div
        className="h-full flex flex-col justify-center space-y-2 cursor-pointer"
        onClick={onPreview}
      >
      ...
      <button
        ...
        onClick={editNote}
      >
        <MdCreate className="text-slate-300 hover:text-green-600" />
      </button>
      <button
        className="absolute right-3 bottom-3 cursor-pointer"
        onClick={deleteNote}
      >
        <MdDelete className="text-slate-300 hover:text-red-500" />
      </button>
    </div>
  );
};
